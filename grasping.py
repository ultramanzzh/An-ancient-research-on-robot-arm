...
class Grasping(object):
    def __init__(self, x, y, z, theta):
        ...
        # cam = camera.RS(self.width, self.height)

        # 获取目标位置x,y,z
        obj_position = np.array([self.x, self.y, self.z])
        print(obj_position)

        # 假定旋转矢量
        rv1 = np.array([0., 0., self.theta])
        ...
        # end_position = [-0.27642892, -0.15970213, 0.49910506, 2.16894895, 2.15278475, 0.08800811]
        # 点与旋转矩阵生成4*4变换矩阵
        ...
        self.newPose = np.array(
            [pv_obj2base[0][0], pv_obj2base[1][0], pv_obj2base[2][0], rv_f[0][0], rv_f[1][0], rv_f[2][0]])
        print(end_position, type(end_position))
        print(self.newPose, type(self.newPose))
